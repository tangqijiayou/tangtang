...
public class KickoutSessionControlFilter extends AccessControlFilter {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		...
		logger.info("session.getAttribute(kickout)" + session.getAttribute("kickout"));
		...
		if (session.getAttribute("kickout") != null) {
			...
			try {
				...
				logger.info("当前用户已经在其他地方登录");

				//返回用户未登录的状态码给前端
				ResponseUtils.out(response, new ResultJsonMsg(ResultJsonMsg.RESULT_CODE_NOTLOGIN, "您已经在其他地方登录，请重新登录！"));
			} catch (Exception e) {
			}
			return false;
		}
